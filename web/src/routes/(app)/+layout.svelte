<script lang="ts">
  ...
  import { ensureSession } from '$lib/stores/auth.svelte';
  ...
  // page in this group by session, so this client-side nav handler
  // only needs to keep the in-memory `_user` cache fresh and connect
  // the SSE stream.
  //
  // Owning these responsibilities here (instead of in the root layout)
  // means the chrome is structurally part of the authenticated app —
  // there's no chance of it unrendering mid-navigation when the auth
  // store transiently flips before the next `ensureSession` completes.
  // That was the previous bug: `{#if isAuthenticated()}` racing
  // navigation events caused the header to flicker off and on.
...
<!--
  Authenticated app shell. Every route under `(app)/` is rendered
  inside this layout. Because the server hook redirects unauthenticated
  visitors to `/login` BEFORE this layout renders, we don't gate the
  chrome on auth state here — it's guaranteed by the routing layer.
-->
